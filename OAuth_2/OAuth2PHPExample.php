...
else
{
    ...
    $_SESSION['token_issued_time'] = time(); // Store when token was issued
    $_SESSION['realm_id'] = $_GET['realmId'] ?? null; // Store realm ID

    header("Location: connected.php?realmId=" . urlencode($_GET['realmId'] ?? '') . "&state=" . urlencode($_GET['state'] ?? ''));
    exit();
